...
Route::middleware(['auth', 'role:administrador'])->group(function () {
    ...
    Route::resource('admin/cursos', CursoController::class)
        ->parameters(['cursos' => 'curso']) // 👈 importante si usas 'curso' como variable en blade
        ->names('cursos');

    Route::delete('admin/cursos/{curso}', [CursoController::class, 'destroy'])->name('cursos.destroy');
    Route::resource('grupos', App\Http\Controllers\GrupoController::class)->middleware(['auth']);
    Route::middleware(['auth'])->group(function () {
    Route::resource('aulas', AulaController::class)->except(['create', 'edit', 'show']);
    });
