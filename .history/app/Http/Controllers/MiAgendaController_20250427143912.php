    ...
    public function confirmarDesdeCorreo(Programacion $programacion)
    {
        if (auth()->guest()) {
            return redirect()->route('login')->with('toast', [
                'type' => 'warning',
                'message' => 'Debes iniciar sesión para confirmar tu asistencia.',
            ]);
        }

        $instructor = $programacion->instructor;

        if (!$instructor || !$instructor->user || auth()->id() !== $instructor->user->id) {
        ...
        return redirect()->route('mi-agenda')->with('success', 'Has confirmado tu asistencia al curso.');
    }
