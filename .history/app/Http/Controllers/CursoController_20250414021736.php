...
class CursoController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Grupos disponibles según coordinación
        ...
        // Cursos visibles según grupo o coordinación
        $cursos = Curso::with('grupos')
            ->when($grupoSeleccionadoId, function ($query) use ($grupoSeleccionadoId) {
                return $query->whereHas('grupos', fn ($q) => $q->where('grupo_id', $grupoSeleccionadoId));
            })
            ->when($coordinacionId, function ($query) use ($coordinacionId) {
                return $query->whereHas('grupos', fn ($q) => $q->where('coordinacion_id', $coordinacionId));
            })
            ->orderBy('nombre')
            ->get();

        // Solo para administrador se muestran todas las coordinaciones
