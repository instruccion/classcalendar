    ...
    public function store(Request $request)
    {
        ...
        try {
            $curso = Curso::create([
                ...
                'coordinacion_id' => auth()->user()->coordinacion_id, // se registra automáticamente
            ]);

            // Asociar curso a grupos (tabla curso_grupo)
            $curso->grupos()->sync($request->grupos);

            return redirect()->back()->with('success', 'Curso registrado exitosamente.');

        } catch (\Exception $e) {
            \Log::error('Error al registrar curso: ' . $e->getMessage());
            return redirect()->back()->with('error', 'No se pudo registrar el curso.');
        }
    }
