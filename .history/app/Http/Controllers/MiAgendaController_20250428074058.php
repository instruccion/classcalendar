...
class MiAgendaController extends Controller
{
    ...
    public function miAgenda()
    {
        ...
        $programaciones = Programacion::with(['curso', 'grupo'])
            ->where('instructor_id', $instructor->id) // usar el ID correcto de instructores
            ->orderBy('fecha_inicio')
            ->get();

        $eventos = $programaciones->map(function ($p) {
            $horaInicio = optional($p->hora_inicio)->format('H:i') ?? '08:00';
            $horaFin = optional($p->hora_fin)->format('H:i') ?? '18:00';
