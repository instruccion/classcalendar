...
class CursoController extends Controller
{
    ...
    public function edit(Curso $curso)
    {
        ...
        // Verifica si el usuario puede editar este curso según su coordinación
        if ($user->rol !== 'administrador' && $curso->coordinacion_id !== $user->coordinacion_id) {
            abort(403, 'No tienes permiso para editar este curso.');
        }

        $gruposDisponibles = ($user->rol === 'administrador')
            ? Grupo::orderBy('nombre')->get()
            : Grupo::where('coordinacion_id', $user->coordinacion_id)->orderBy('nombre')->get();
    ...
    {
        $curso->grupos()->detach(); // Limpiar relación many-to-many
        $curso->delete();
