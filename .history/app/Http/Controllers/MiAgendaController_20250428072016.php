...
class MiAgendaController extends Controller
{
    ...
    public function miAgenda()
    {
        ...
        // Aquí debes usar $instructor->id, no $usuario->id
        $programaciones = Programacion::with(['curso', 'grupo'])
            ->where('instructor_id', $instructor->id) // <--- corregido
            ->orderBy('fecha_inicio')
            ->get();
        ...
        $eventos = $programaciones->map(function ($p) {
            return [
                'title' => $p->curso->nombre ?? 'Curso sin título',
                'start' => $p->fecha_inicio?->format('Y-m-d') ?? '2025-01-01',
                'end'   => $p->fecha_fin?->format('Y-m-d') ?? '2025-01-01',
                'color' => '#3788D8', // Opcionalmente podrías usar color dinámico
            ];
        });
