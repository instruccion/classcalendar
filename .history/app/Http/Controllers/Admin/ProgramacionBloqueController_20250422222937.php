...
class ProgramacionBloqueController extends Controller
{
    ...
    public function updateBloque(Request $request, Grupo $grupo, $bloque_codigo = null)
    {
        $validated = $request->validate([
            ...
            'cursos.*.fecha_inicio' => 'required|date',
            'cursos.*.hora_inicio' => 'required|date_format:H:i',
            'cursos.*.fecha_fin' => 'required|date',
            'cursos.*.hora_fin' => 'required|date_format:H:i',
            'cursos.*.programacion_id' => 'nullable|integer'
        ]);
        ...
        try {
            ...
            // Eliminar programaciones anteriores del bloque
            $query = Programacion::where('grupo_id', $grupo->id);
            if ($bloque_codigo === '_sin_codigo_') {
                $query->whereNull('bloque_codigo');
            } else {
            ...
            foreach ($validated['cursos'] as $cursoData) {
                Programacion::create([
                    ...
                    'instructor_id' => null,
                    'bloque_codigo' => $bloqueCodigo,
                    'observaciones' => 'Actualizado por edición de bloque'
                ]);
            }
            ...
            DB::rollBack();
            report($e);
            return back()->with('error', 'Error al actualizar el bloque.');
        }
    }
}
