    ...
    public function calcularFechaFinApi(Request $request)
    {
        ...
        try {
            ...
            while ($minutosAcumulados < $minutosTotales) {
                ...
                $rango = [
                    [$fechaActual->copy()->setTime(8, 30), $fechaActual->copy()->setTime(12, 0)],
                    [$fechaActual->copy()->setTime(13, 0), $fechaActual->copy()->setTime(17, 0)],
                ];

                foreach ($rango as [$inicio, $fin]) {
                    if ($fechaActual->lt($fin)) {
                        if ($fechaActual->lt($inicio)) $fechaActual->setTimeFrom($inicio);
                        $disponible = $fechaActual->diffInMinutes($fin);
                        $usar = min($minutosTotales - $minutosAcumulados, $disponible);
                        $fechaActual->addMinutes($usar);
                        $minutosAcumulados += $usar;
                        if ($minutosAcumulados >= $minutosTotales) break 2;
                    }
                }
            ...
            return response()->json(['error' => 'No se pudo calcular fecha fin.'], 500);
        }
    }
}
