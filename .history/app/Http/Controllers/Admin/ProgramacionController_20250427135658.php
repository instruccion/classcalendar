...
use App\Mail\InvitacionCursoInstructor;
use App\Mail\ConfirmacionInstructorMail;
...
class ProgramacionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Solo si hay instructor asignado
        if ($programacion->instructor && $programacion->instructor->correo) {
            // Generar un token único para confirmar
            $programacion->token_confirmacion = \Illuminate\Support\Str::uuid();
            $programacion->save();

            // Enviar correo al instructor
            Mail::to($programacion->instructor->correo)
                ->send(new \App\Mail\ConfirmacionInstructorMail($programacion));
        }
    ...
    public function update(Request $request, Programacion $programacion)
    {
        ...
        try {
            ...
            // Detectar cambio de instructor
            if ($validated['instructor_id'] && $validated['instructor_id'] != $programacion->getOriginal('instructor_id')) {
                $nuevoInstructor = Instructor::find($validated['instructor_id']);

                if ($nuevoInstructor && $nuevoInstructor->correo) {
                    // Generar un nuevo token único
                    $token = Str::uuid();
                    $programacion->token_confirmacion = $token;
                    $programacion->estado_confirmacion = 'pendiente';
                    $programacion->fecha_confirmacion = null;
                    $programacion->motivo_rechazo = null;
                    $programacion->save();

                    // Enviar correo al nuevo instructor
                    Mail::to($nuevoInstructor->correo)->send(new ConfirmacionInstructorMail($programacion));
                }
            }
    ...
    public function enviarCorreo(Programacion $programacion)
    {
        if (!$programacion->instructor || !$programacion->instructor->correo) {
            return back()->with('error', 'El instructor no tiene correo asignado.');
        }

        Mail::to($programacion->instructor->correo)
            ->send(new InvitacionCursoInstructor($programacion));

        return back()->with('success', 'Correo enviado al instructor.');
    }
