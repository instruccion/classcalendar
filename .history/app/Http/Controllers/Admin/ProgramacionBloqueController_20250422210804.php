...
class ProgramacionBloqueController extends Controller
{
    ...
    public function editBloque(Request $request, Grupo $grupo, $bloque_codigo = null)
    {
        ...
        $cursosDisponibles = $grupo->cursos()
        ->select('cursos.id', 'cursos.nombre', 'cursos.tipo', 'cursos.duracion_horas')
        ...
        return view('admin.programaciones.bloque.edit', compact(
            ...
            'aulaActualId',
            'instructorActualId',
            'cursosDisponibles' // 👈 Agregado aquí
        ));
    ...
    {
         Log::info("Intentando actualizar bloque", ['grupo' => $grupo->id, 'codigo_original' => $bloque_codigo, 'data' => $request->all()]);
         // TODO: Implementar validación, eliminación de antiguas, cálculo, verificación y creación de nuevas.
         dd('Método updateBloque alcanzado. Datos recibidos:', $request->all(), $grupo, $bloque_codigo);
         // return redirect()->route('admin.programaciones.index')->with('success', 'Bloque actualizado (simulado)');
    }
}
