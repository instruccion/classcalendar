...
class ProgramacionController extends Controller
{
    ...
    public function calcularFechaFinApi(Request $request)
    {
        ...
        try {
            ...
            $minutosAcumulados = 0;
            $minutosNecesarios = $minutosTotalesRequeridos;
            ...
            while ($minutosAcumulados < $minutosNecesarios) {
                ...
                $minutosPorAsignarEsteCiclo = $minutosNecesarios - $minutosAcumulados;
                ...
                if ($fechaActual->lt($finManana)) {
                    ...
                    if ($minutosAUsar > 0) {
                        ...
                        $minutosPorAsignarEsteCiclo -= $minutosAUsar;
                        if ($minutosAcumulados >= $minutosNecesarios) break;
                    }
                }
            ...
            $query = Programacion::where($columnaId, $resourceId)
                ->where(function ($q) use ($inicio, $fin) {
                    $q->whereRaw('TIMESTAMP(fecha_inicio, hora_inicio) < ?', [$fin])
                      ->whereRaw('TIMESTAMP(fecha_fin, hora_fin
