...
class ProgramacionController extends Controller
{
     ...
     */
     public function showProgramarBloque()
     {
        ...
        // --- CONTINUACIÓN: MÉTODOS PARA LAS RUTAS API ---

        /**
         * Obtiene los instructores activos que pueden dictar un curso específico.
         * Responde en formato JSON para llamadas Fetch/Alpine.
         *
         * @param \App\Models\Curso $curso El modelo Curso inyectado.
         * @return \Illuminate\Http\JsonResponse
         */
        public function getInstructoresPorCursoApi(Curso $curso)
        {
            try {
                // Asume que tienes una relación 'instructores' en tu modelo Curso
                // que devuelve solo los instructores asociados Y activos.
                // Si no, ajusta la consulta.
                $instructores = $curso->instructores() // Necesita relación en modelo Curso
                                      ->where('activo', true) // Asegurarse que estén activos
                                      ->select('instructores.id', 'instructores.nombre')
                                      ->orderBy('instructores.nombre')
                                      ->get();

                // Devuelve los instructores como JSON
                return response()->json($instructores);

            } catch (\RelationNotFoundException $e) {
                 // Error común si la relación 'instructores' no existe en el modelo Curso
                 \Log::error("Error: Relación 'instructores' no encontrada en el modelo Curso. ID: " . $curso->id);
                 return response()->json(['error' => "Relación de instructores no definida para este curso."], 500);
            } catch (\Exception $e) {
                // Otros errores
                \Log::error("Error al obtener instructores por curso (ID: {$curso->id}): " . $e->getMessage());
                return response()->json(['error' => 'Error interno al obtener los instructores.'], 500);
            }
        }

        /**
         * Calcula la fecha y hora de fin estimada para una programación.
         * Responde en formato JSON para llamadas Fetch/Alpine.
         *
         * @param Request $request Espera 'inicio' (Y-m-d), 'horas' (int), 'hora_inicio' (H:i)
         * @return \Illuminate\Http\JsonResponse
         */
        public function calcularFechaFinApi(Request $request)
        {
             ...
             try {
                 // --- Lógica de Cálculo (Simplificada por ahora, la haremos compleja después) ---
                 // TODO: Implementar lógica detallada con 45min/h, horarios, feriados.
                 // Por ahora, solo sumamos días completos para probar la API.

                 $fechaInicio = Carbon::parse($validated['inicio'] . ' ' . $validated['hora_inicio']);
                 // Cálculo MUY BÁSICO: asume 8h académicas por día (necesitamos ajustar)
                 $diasNecesarios = ceil($validated['horas'] / 8); // Muy simplificado

                 // Cargar feriados (simplificado, idealmente cachear)
                 ...
                 while ($diasHabilesContados < $diasNecesarios) {
                     // Si es el primer día, ya cuenta si es hábil
                     if ($diasHabilesContados > 0) {
                          $fechaFin->addDay();
                     }

                     // Saltar fines de semana y feriados
                     while ($fechaFin->isWeekend() || in_array($fechaFin->format('Y-m-d'), $feriados)) {
                         $fechaFin->addDay();
                     }
                     $diasHabilesContados++;
                 }

                 // Hora fin tentativa (simplificada, debe ser calculada con detalle)
                 $horaFin = '17:00'; // Placeholder

                 return response()->json([
                     'fecha_fin' => $fechaFin->format('Y-m-d'),
                     'hora_fin' => $horaFin // Placeholder, necesitamos calcularla bien
                 ]);

             } catch (\Exception $e) {
                 \Log::error("Error al calcular fecha fin: " . $e->getMessage());
                 return response()->json(['error' => 'Error interno al calcular la fecha de fin.'], 500);
             }
        }
