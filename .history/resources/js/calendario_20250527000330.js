...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (calendarEl) {
        ...
        // Filtrar grupos cuando se selecciona una coordinación
        if (coordSelect) {
            coordSelect.addEventListener('change', function () {
                const coordId = coordSelect.value;
                const grupoSelect = document.getElementById('grupo');

                // Si no hay grupo select, salir
                ...
                if (!coordId) {
                    // Si se selecciona "Todas", no cargar nada adicional
                    return;
                }

                // Consultar grupos de esa coordinación
                fetch(`/cursoslaser/public/admin/grupos-por-coordinacion/${coordId}`)
                    ...
                    .then(data => {
                        ...
                        // Luego de cargar nuevos grupos, refrescar calendario
                        reloadCalendar();
                    })
                    .catch(err => {
