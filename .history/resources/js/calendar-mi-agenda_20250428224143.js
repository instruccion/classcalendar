...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const calendar = new Calendar(calendarEl, {
        ...
        events: function(fetchInfo, successCallback, failureCallback) {
            let apiUrl = window.location.pathname.replace('/mi-agenda', '/api/mi-agenda');
            if (apiUrl.includes('index.php')) {
                apiUrl = apiUrl.replace('/index.php', '');
            }
            console.log('URL FINAL de fetch:', apiUrl + '?instructor_id=' + instructorId);

            fetch(apiUrl + '?instructor_id=' + instructorId)
