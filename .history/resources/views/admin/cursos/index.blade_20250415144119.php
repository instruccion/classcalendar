<x-app-layout>
                    ...
                    @forelse ($cursos as $curso)
                        <tr class="border-t">
                            ...
                            <td class="px-4 py-2 flex gap-2">
                                ...
                                <form action="{{ route('admin.cursos.destroy', $curso) }}" method="POST" onsubmit="return confirm('Confirma la eliminación del curso?')">
                                    @csrf
                                    @method('DELETE')
    ...
    <script>
        ...
        function updateGrupos() {
            ...
            // Si seleccionamos "Todas" no hacemos ninguna petición, simplemente limpiamos los grupos
            if (coordinacion_id === "") {
                document.getElementById('grupo').innerHTML = '<option value="">Todos los grupos</option>';
                return;
            }

            // Realizar una petición AJAX para obtener los grupos de la coordinación seleccionada
            fetch(`/admin/grupos-por-coordinacion?coordinacion_id=${coordinacion_id}`)
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (coordinacionSelect) {
                coordinacionSelect.addEventListener('change', function () {
                    const id = this.value;

                    // Si seleccionamos "Todas", no hacer ninguna petición y solo limpiar los grupos
                    if (id === "") {
                        grupoSelect.innerHTML = '<option value="">Todos los grupos</option>';
                        return;
                    }

                    // Si no es "Todas", hacemos la petición para cargar los grupos relacionados con la coordinación
                    grupoSelect.innerHTML = '<option value="">Todos los grupos</option>'; // Limpiar las opciones actuales

                    fetch(`/admin/grupos-por-coordinacion?coordinacion_id=${id}`)
                        .then(response => response.json())
                        .then(data => {
                            data.grupos.forEach(grupo => {
