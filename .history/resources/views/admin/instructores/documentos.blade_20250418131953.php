...
<script>
 function abrirModalEditar(pivot_id, nombre, fecha) {
        ...
        // ✅ CONSTRUCCIÓN SEGURA para evitar doble index.php
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        if (!form) {
            alert("❌ No se encontró el formulario con ID 'formEditarDocumento'");
            return;
        }

        const currentAction = form.getAttribute("action");
        alert("🔎 Action actual del formulario: " + currentAction);

        form.addEventListener("submit", (e) => {
            const method = form.querySelector("input[name='_method']")?.value || "NO _method DEFINIDO";
            const fecha = document.getElementById("fecha_vencimiento_editar").value;
            alert("✅ Enviando con método: " + method + " y fecha: " + fecha);
        });
    });


    document.addEventListener("DOMContentLoaded", () => {
        console.log("✅ Página cargada desde: ", window.location.href);
    });
