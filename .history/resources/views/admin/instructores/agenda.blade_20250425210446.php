    ...
    <dialog id="detalleCursoModal" class="w-full max-w-xl rounded shadow backdrop:bg-black/40">
        <div class="bg-white p-6 space-y-3">
            <div class="flex justify-between items-center border-b pb-2">
                ...
                <button onclick="document.getElementById('detalleCursoModal').close()" class="text-gray-600 hover:text-black text-xl">×</button>
            </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                events: async function (fetchInfo, successCallback, failureCallback) {
                    try {
                        const response = await fetch(`/api/mi-agenda?instructor_id=${instructorId}`);
