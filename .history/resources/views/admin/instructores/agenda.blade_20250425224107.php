        ...
        const calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            events: async function (fetchInfo, successCallback, failureCallback) {
                try {
                    ...
                    const url = `/cursoslaser/public/index.php/api/mi-agenda${instructorId ? '?instructor_id=' + instructorId : ''}`;

                    const response = await fetch(url);
                    const data = await response.json();

                    console.log('Eventos cargados:', data); // <-- solo para depuración

                    const eventos = data.map(item => ({
                        ...item // preserva todos los campos: title, start, end, color, etc.
                    }));
            ...
            eventDisplay: 'block', // <-- importante para que el color se aplique en toda la celda
        });
