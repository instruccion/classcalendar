...
<x-app-layout>
    ...
    function abrirModalEditar(pivot_id, nombre, fechaYmd) { // Renombrado fecha a fechaYmd para claridad
        ...
        console.log("--- Abrir Modal ---"); // Log inicio
        console.log("Pivot ID recibido:", pivot_id); // Log pivot_id
        console.log("Nombre recibido:", nombre);
        console.log("Fecha YMD recibida:", fechaYmd);

        // Construcción de la URL para el action del formulario
        const baseUrlString = @json(rtrim(url('admin/instructores/documentos'), '/'));
        console.log("Base URL desde Blade:", baseUrlString); // Log URL base

        // Verificar que pivot_id no sea undefined o null
        if (!pivot_id) {
            console.error("¡ERROR: pivot_id es inválido!", pivot_id);
            alert("Error: No se pudo obtener el ID del documento para editar.");
            return; // Detener ejecución si pivot_id es inválido
        }

        const fullAction = `${baseUrlString}/${pivot_id}`;
        console.log("URL de acción calculada:", fullAction); // Log URL completa

        try {
            form.setAttribute('action', fullAction);
            console.log("Atributo 'action' establecido en el form:", form.getAttribute('action')); // Verificar después de setear
        } catch (error) {
        ...
        try {
            ...
            document.getElementById('fecha_vencimiento_editar').value = fechaYmd ?? '';
