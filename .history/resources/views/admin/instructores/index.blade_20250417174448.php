<x-app-layout>
            ...
            <table class="min-w-full text-sm border">
                ...
                <tbody>
                    @forelse($instructores as $instructor)
                        @php
                            $tieneVencido = $instructor->documentosVencidos()?->count() > 0;
                        @endphp
                        <tr class="border-t hover:bg-gray-50">
                            <td class="px-4 py-2 font-semibold {{ $tieneVencido ? 'text-red-600' : 'text-gray-800' }}">
                                {{ $instructor->nombre }}
                            </td>
                            <td class="px-4 py-2">{{ $instructor->correo ?: '—' }}</td>
                            <td class="px-4 py-2">{{ $instructor->telefono ?: '—' }}</td>
                            <td class="px-4 py-2">{{ $instructor->coordinaciones->pluck('nombre')->join(', ') ?: '—' }}</td>
                            <td class="px-4 py-2">{{ $instructor->cursos->pluck('nombre')->join(', ') ?: '—' }}</td>
                            <td class="px-4 py-2 flex flex-wrap gap-2">
                                <button onclick='editarInstructor(@json($instructor->only(["id", "nombre", "especialidad", "correo", "telefono"])) , @json($instructor->coordinaciones->pluck("id")), @json($instructor->cursos->pluck("id")))' class="text-blue-600 hover:underline">Editar</button>

                                <a href="{{ route('admin.instructores.documentos', $instructor) }}" class="text-indigo-600 hover:underline">Documentos</a>
    ...
    <script>
        ...
        function editarInstructor(data, coordIds, cursoIds) {
            const form = document.getElementById('formInstructor');
            ...
            [...coordSel.options].forEach(option => {
                option.selected = coordIds.includes(Number(option.value));
            });

            const cursoSel = document.getElementById('curso_ids');
            [...cursoSel.options].forEach(option => {
                option.selected = cursoIds.includes(Number(option.value));
            });
