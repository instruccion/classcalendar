<x-app-layout>
    ...
    <div class="py-6 max-w-7xl mx-auto space-y-6">
        ...
        <form id="form-instructor" class="mb-4 max-w-md">
    ...
    <dialog id="detalleCursoModal" class="w-full max-w-xl rounded shadow backdrop:bg-black/40">
        <div class="bg-white p-6 space-y-3">
            <div class="flex justify-between items-center border-b pb-2">
                <h2 class="text-xl font-bold text-gray-800">📘 Detalles del Curso</h2>
                <button onclick="document.getElementById('detalleCursoModal').close()"
                        class="text-gray-600 hover:text-black text-xl">×</button>
            </div>
            <div id="detalleContenido" class="text-sm text-gray-700 space-y-1"></div>
        </div>
    </dialog>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function cargarEventos(instructorId = '') {
                if (calendar) {
                    calendar.destroy(); // 🔥 Importante: destruimos si ya existe para evitar duplicados
                }
                ...
                calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    events: async function (fetchInfo, successCallback, failureCallback) {
                        try {
                            const url = `/api/mi-agenda?instructor_id=${instructorId}`;
                            const response = await fetch(url);
                            const data = await response.json();
            ...
            cargarEventos('{{ request('instructor_id') }}'); // 🔥 Primera carga

            // Manejar cambios de instructor
            ...
            selectInstructor.addEventListener('change', function () {
                ...
                fetch(`{{ route('admin.instructores.agenda') }}?instructor_id=${selectedInstructorId}`)
                    .then(res => res.text())
                    .then(html => {
                        const parser = new DOMParser();
                        const doc = parser.parseFromString(html, 'text/html');
                        const tablaNueva = doc.querySelector('.bg-white.shadow.p-4.rounded.overflow-x-auto');

                        if (tablaNueva) {
                            const tablaActual = document.querySelector('.bg-white.shadow.p-4.rounded.overflow-x-auto');
                            if (tablaActual && tablaActual.parentNode) {
                                tablaActual.parentNode.replaceChild(tablaNueva, tablaActual);
                            }
                        }
                    });

                // Recargar eventos del calendario
                cargarEventos(selectedInstructorId);
            });
        });
        ...
        eventClick: function(info) {
            ...
            const detalleHTML = `
                ...
                <p><strong>Inicio:</strong> ${props.fecha_inicio}</p>
                <p><strong>Fin:</strong> ${props.fecha_fin}</p>
            ...
            document.getElementById('detalleCursoModal').showModal();
        }
