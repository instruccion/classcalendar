<x-app-layout>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            async function loadCoursesForGroup(groupId) {
                cursoSelect.innerHTML = '<option selected disabled>Cargando cursos...</option>';

                if (!groupId) {
                    cursoSelect.innerHTML = '<option selected disabled>Seleccione un grupo primero</option>';
                    return;
                }

                const url = `/admin/api/grupos/${groupId}/cursos`;
                ...
                try {
                    ...
                    if (!Array.isArray(data) || data.length === 0) {
                        cursoSelect.innerHTML = '<option selected disabled>No hay cursos para este grupo...</option>';
                        return;
                    }
                    ...
                    data.forEach(curso => {
                        ...
                        opt.value = curso.id;
                        opt.textContent = `${curso.nombre} (${curso.duracion_horas}h)`;
                        cursoSelect.appendChild(opt);
                    });
                } catch (error) {
                    console.error('Error al cargar cursos:', error);
                    cursoSelect.innerHTML = '<option selected disabled>Error al cargar cursos</option>';
                }
            }

            if (grupoSelect) {
                loadCoursesForGroup(grupoSelect.value); // Por si ya hay uno seleccionado
                grupoSelect.addEventListener('change', () => {
                    loadCoursesForGroup(grupoSelect.value);
                });
            }
        });
    </script>
