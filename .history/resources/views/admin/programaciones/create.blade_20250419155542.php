<x-app-layout>
    ...
    {{-- CSS de FullCalendar (Cargado vía Vite/app.css, no se necesita CDN aquí) --}}
    ...
    <div class="py-6 max-w-4xl mx-auto">
                ...
                return {
                    ...
                    openAvailabilityModal(type) {
                        ...
                        if (!modal || typeof modal.showModal !== 'function') {
                            console.error("Alpine: Modal <dialog> no encontrado."); return;
                        }

                        // 1. Poner en estado de carga y mostrar el modal
                        this.isLoadingAvailability = true;
                        modal.showModal();
                        // Limpiar instancia previa del calendario si existe
                        if (this.calendarInstance) {
                            this.calendarInstance.destroy();
                            this.calendarInstance = null;
                        }
                        // Limpiar contenido previo (se hará dentro del fetch ahora)
                        // document.getElementById('mini-calendar').innerHTML = ''; // No hacerlo aquí
                        // document.getElementById('tabla-detalle-contenido').innerHTML = '<p>Cargando...</p>'; // No hacerlo aquí


                        // 2. Preparar y hacer la llamada Fetch
                        ...
                        fetch(url)
                            ...
                            .then(data => {
                                ...
                                // 3. ¡IMPORTANTE! Poner isLoadingAvailability a false ANTES de buscar los elementos
                                this.isLoadingAvailability = false;
                                console.log("Alpine: openAvailabilityModal - isLoadingAvailability puesto a false.");

                                // 4. Usar $nextTick para asegurar que el DOM se actualizó por x-if
                                this.$nextTick(() => {
                                    console.log("Alpine: openAvailabilityModal ($nextTick) - DOM actualizado, buscando elementos...");
                                    const calendarEl = document.getElementById('mini-calendar');
                                    const tablaDetalleContenido = document.getElementById('tabla-detalle-contenido');

                                    // Verificar si los elementos existen AHORA
                                    if (!calendarEl || !tablaDetalleContenido) {
                                        console.error("Alpine: openAvailabilityModal ($nextTick) - #mini-calendar o #tabla-detalle-contenido NO encontrados DESPUÉS de $nextTick!");
                                        return; // Salir si aún no existen
                                    }
                                    console.log("Alpine: openAvailabilityModal ($nextTick) - Elementos encontrados.");
                                    calendarEl.innerHTML = ''; // Limpiar ahora sí
                                    tablaDetalleContenido.innerHTML = ''; // Limpiar ahora sí

                                    // 5. Renderizar Calendario y Tabla (dentro de $nextTick)
                                    try {
                                        if (typeof window.FullCalendar === 'undefined' || !window.FullCalendar) throw new Error('FullCalendar no está en window.');
                                        console.log("Alpine: openAvailabilityModal ($nextTick) - Inicializando FullCalendar...");
                                        this.calendarInstance = new window.FullCalendar(calendarEl, { /* ... Opciones ... */ });
                                        this.calendarInstance.render();
                                        console.log("Alpine: openAvailabilityModal ($nextTick) - Calendario OK.");

                                        const detalles = data.tabla || [];
                                        if (detalles.length === 0) {
                                            tablaDetalleContenido.innerHTML = '<p class="text-gray-500 text-sm text-center mt-4">No hay cursos programados.</p>';
                                        } else {
                                        ...
                                        console.log("Alpine: openAvailabilityModal ($nextTick) - Tabla OK.");

                                    } catch (renderError) {
                                        console.error("Alpine: openAvailabilityModal ($nextTick) ¡ERROR RENDER!", renderError);
                            ...
                            .catch(error => {
                                ...
                                // Asegurarse de quitar el "cargando" incluso si falla el fetch
                                this.isLoadingAvailability = false;
                                // Mostrar error en el contenedor principal del modal si es posible
                                const contentContainer = modal.querySelector('.grid'); // Busca el grid interno
                                if(contentContainer) {
                                    contentContainer.innerHTML = `<p class="text-red-500 text-center p-4 col-span-2">Error al cargar datos: ${error.message}</p>`;
                                }
                            });
        ...
        <div class="bg-white p-6 rounded shadow-md"
            ...
            <form x-bind:action="formAction" method="POST" id="form-programacion" @submit.prevent="submitForm"
                ...
                {{-- Este HTML no necesita cambios respecto a tu versión anterior --}}
                 <!-- Grupo -->
                 <div>
        ...
        <dialog id="modalDisponibilidad" ...>
            <div class="bg-white p-6 relative">
                <button @click="document.getElementById('modalDisponibilidad').close()" ...>×</button> {{-- Usar cierre directo --}}
                <h2 ...>Disponibilidad</h2>

                {{-- Mensaje de Carga con x-if --}}
                <template x-if="isLoadingAvailability">
                    <div class="text-center py-10">Cargando disponibilidad...</div>
                </template>
