<x-app-layout>
    <div class="py-6 max-w-7xl mx-auto">
        ...
        <form method="GET" action="{{ route('admin.programaciones.index') }}"
        ...
        <div class="bg-white rounded shadow overflow-x-auto">
            @forelse ($programacionesAgrupadas as $grupoNombre => $bloques)
                ...
                <div class="px-4 py-2 text-sm bg-gray-50 border-b text-gray-700">
                    <span class="font-semibold italic underline">Bloque:</span>
                    <span class="italic underline">{{ $bloqueCodigo ?: '—' }}</span>
                    <div class="px-4 py-2 text-sm bg-gray-50 border-b text-gray-700 flex justify-between items-center"> {{-- Añadido flex --}}
                        <div> {{-- Contenedor para el título --}}
                            <span class="font-semibold italic underline">Bloque:</span>
                            <span class="italic underline">{{ $bloqueCodigo ?: '—' }}</span>
                        </div>
                        <div> {{-- Contenedor para el botón --}}
                            {{-- Enlace para Editar el Bloque --}}
                            <a href="{{ route('admin.programaciones.bloque.edit', ['grupo' => $items->first()->grupo_id, 'bloque_codigo' => $bloqueCodigo ?: '_sin_codigo_']) }}" {{-- Usamos _sin_codigo_ si es null --}}
