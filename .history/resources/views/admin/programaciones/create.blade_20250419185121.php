                ...
                return {
                    ...
                    // --- Modal de Disponibilidad (Simplificado, usando DIV y estado Alpine) ---
                        ...
                        openAvailabilityModal(type) {
                            const resourceId = type === 'instructor' ? this.selectedInstructorId : this.selectedAulaId;
                            if (!resourceId) { alert('Seleccione recurso'); return; } // Salir si no hay ID

                            const resourceName = document.querySelector(`#${type}_id option:checked`)?.textContent || `Recurso ID ${resourceId}`;
                            const modalDiv = document.getElementById('modalDisponibilidad');
                            const modalTituloEl = document.getElementById('modalTitulo');
                            const tablaDetalleContenido = document.getElementById('tabla-detalle-contenido'); // Mantenemos la tabla

                            if (!modalDiv || !tablaDetalleContenido || !modalTituloEl) {
                                console.error("Alpine: Elementos del modal simplificado no encontrados."); return;
                            }

                            console.log(`Alpine: Abriendo modal simplificado para ${type} ID: ${resourceId}`);
                            ...
                            this.isLoadingAvailability = true; // Mostrar "Cargando..." inicialmente
                            tablaDetalleContenido.innerHTML = '<p>Cargando detalles...</p>'; // Limpiar tabla

                            // Simular una carga y luego mostrar contenido simple
                            setTimeout(() => {
                                console.log("Alpine: Simulando fin de carga.");
                                const detalles = []; // Simular datos vacíos por ahora
                                if (detalles.length === 0) {
                                    tablaDetalleContenido.innerHTML = '<p class="text-gray-500 text-sm text-center mt-4">No hay cursos programados (prueba simplificada).</p>';
                                } else {
                                    // Aquí iría la lógica para renderizar la tabla si hubiera datos
                                    tablaDetalleContenido.innerHTML = 'Tabla iría aquí';
                                }
                                this.isLoadingAvailability = false; // Ocultar "Cargando..."
                                console.log("Alpine: isLoadingAvailability puesto a false.");
                            }, 1000); // Simular 1 segundo de carga

                        }, // Fin openAvailabilityModal (simplificada)
