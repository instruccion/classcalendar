    ...
    <div class="py-6 max-w-4xl mx-auto">
        ...
        <script>
            function programacionForm(config) {
                return {
                    ...
                    formMethod: 'POST',
                    formAction: config.rutasApi.store,
                    calendarInstance: null,
                    isModalOpen: false, // Para controlar modal disponibilidad
                    isEventModalOpen: false, // Para controlar modal evento
                    ...
                    loadCourses() {
                        this.selectedCourseId = ''; this.availableCourses = []; this.selectedInstructorId = ''; this.availableInstructors = []; this.endDate = ''; this.endTime = ''; this.instructorOccupied = false; this.aulaOccupied = false;
                        if (!this.selectedGroupId) return;
                        this.isLoadingCourses = true; const url = this.rutasApi.cursosPorGrupo.replace(':grupoId', this.selectedGroupId);
                        fetch(url)
                            .then(res => res.ok ? res.json() : Promise.reject('Error API Cursos'))
                            .then(data => this.availableCourses = data)
                            .catch(error => { console.error("Alpine: Error loadCourses:", error); this.availableCourses = []; })
                            .finally(() => this.isLoadingCourses = false);
                    },
                    handleCourseChange() {
                        this.selectedInstructorId = ''; this.availableInstructors = []; this.instructorOccupied = false;
                        this.loadInstructors(); this.calculateEndDateAndCheckAvailability();
                    },
                    loadInstructors() {
                        this.availableInstructors = []; if (!this.selectedCourseId) return;
                        this.isLoadingInstructors = true; const url = this.rutasApi.instructoresPorCurso.replace(':cursoId', this.selectedCourseId);
                        fetch(url)
                            .then(res => res.ok ? res.json() : res.json().then(err => { throw new Error(err.error || `Error HTTP ${res.status}`) }))
                            .then(data => this.availableInstructors = data)
                            .catch(error => { console.error("Alpine: Error loadInstructors:", error); this.availableInstructors = []; })
                            .finally(() => this.isLoadingInstructors = false);
                    },
                    calculateEndDateAndCheckAvailability() { this.calculateEndDate(); },
                    calculateEndDate() {
                        this.endDate = ''; this.endTime = ''; this.calculatedEndDateText = ''; this.showEndDateToast = false;
                        const duration = this.selectedCourseDuration;
                        if (!this.startDate || !duration || duration <= 0) return;
                        this.isLoadingEndDate = true;
                        fetch(this.rutasApi.calcularFechaFin, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': this.csrfToken, 'Accept': 'application/json' }, body: JSON.stringify({ inicio: this.startDate, horas: duration, hora_inicio: this.startTime }) })
                        .then(res => res.ok ? res.json() : res.json().then(err => { throw new Error(err.error || `Error HTTP ${res.status}`) }))
                        .then(data => { if (data.fecha_fin && data.hora_fin) { this.endDate = data.fecha_fin; this.endTime = data.hora_fin; this.calculatedEndDateText = `${data.fecha_fin} ${data.hora_fin}`; this.showEndDateToast = true; setTimeout(() => this.showEndDateToast = false, 3500); } else { console.error("Alpine: Respuesta inesperada calc fecha:", data); alert("No se pudo calcular fecha/hora fin."); }})
                        .catch(error => { console.error("Alpine: Error fetch calc fecha:", error); alert(`Error calculando fecha fin: ${error.message}`); this.endDate = ''; this.endTime = ''; })
                        .finally(() => { this.isLoadingEndDate = false; this.checkAvailability('instructor'); this.checkAvailability('aula'); });
                    },
                    checkAvailability(type) {
                        const resourceId = type === 'instructor' ? this.selectedInstructorId : this.selectedAulaId;
                        const startDate = this.startDate; const endDate = this.endDate; const startTime = this.startTime; const endTime = this.endTime;
                        if(type === 'instructor') this.instructorOccupied = false; else this.aulaOccupied = false;
                        if (!resourceId || !startDate || !endDate || !startTime || !endTime) return;
                        ...
                        fetch(url)
                            ...
                            .then(data => { if (type === 'instructor') this.instructorOccupied = data.ocupado; else this.aulaOccupied = data.ocupado; })
                    ...
                    openAvailabilityModal(type) {
                        ...
                        const modal = document.getElementById('modalDisponibilidad'); // Usaremos este ID para el DIV modal
                        const calendarEl = document.getElementById('mini-calendar');
                        const tablaDetalleContenido = document.getElementById('tabla-detalle-contenido');
                        if (!modal || !calendarEl || !tablaDetalleContenido) { console.error("Alpine: Modal o elementos internos no encontrados."); return; }
                        ...
                        calendarEl.innerHTML = ''; tablaDetalleContenido.innerHTML = '<p>Cargando...</p>';
                        ...
                        fetch(url)
                                ...
                                requestAnimationFrame(() => {
                                    try {
                                        ...
                                        this.calendarInstance = new window.FullCalendar(calendarEl, {
                                            ...
                                            eventClick: (info) => {
                                                const props = info.event.extendedProps; const self = this; // Guardar contexto
                                                ...
                                                self.isEventModalOpen = true; // Usar contexto guardado
                                            }
                                        });
                                        ...
                                        if (detalles.length === 0) { tablaDetalleContenido.innerHTML = '<p class="text-gray-500 text-sm text-center mt-4">No hay cursos programados.</p>'; }
                                         ...
                                         console.error("Alpine: ¡ERROR RENDER!", renderError);
                            ...
                            .catch(error => {
                                 console.error(`Alpine: ERROR fetch (${type}):`, error); this.isLoadingAvailability = false;
                    ...
                    submitForm(event) {
                         console.log('Alpine: Validando...'); if (!this.selectedGroupId) { alert('Seleccione Grupo.'); return; } if (!this.selectedCourseId) { alert('Seleccione Curso.'); return; } if (!this.startDate) { alert('Seleccione Fecha Inicio.'); return; } if (!this.endDate) { alert('Falta Fecha Fin.'); return; } if (!this.selectedAulaId) { alert('Seleccione Aula.'); return; } if (!this.selectedInstructorId) { alert('Seleccione Instructor.'); return; } if (this.aulaOccupied) { if (!confirm('ADVERTENCIA: Aula ocupada. ¿Continuar?')) return; } if (this.instructorOccupied) { if (!confirm('ADVERTENCIA: Instructor ocupado. ¿Continuar?')) return; } console.log('Alpine: Enviando...'); event.target.submit();
                     }
                } // Fin return
            } // Fin programacionForm
        </script>
        {{-- ============================================= --}}
        {{-- FIN SCRIPT ALPINE --}}
        {{-- ============================================= --}}
        ...
        <div class="bg-white p-6 rounded shadow-md"
             x-data="programacionForm({
              ...
              {{-- Listener para cerrar modales (si usamos $dispatch) --}}
             {{-- @close-dialog.window="document.getElementById($event.detail)?.close()" --}}
             x-init="init()">
            ...
            <form x-bind:action="formAction" method="POST" id="form-programacion" @submit.prevent="submitForm"
                ...
                <input type="hidden" name="_method" x-bind:value="formMethod">
                 ...
                 <!-- Campos del formulario (Grupo, Curso, Bloque, Fechas, Horas, Aula, Instructor, Botón) -->
                 <!-- Grupo -->
                 <div>
