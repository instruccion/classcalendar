<x-app-layout>
    ...
    {{-- CSS de FullCalendar (Asegúrate que se cargue vía Vite/app.css) --}}
    {{-- NO necesitas link CDN aquí si lo importas en app.css --}}
    ...
    <div class="py-6 max-w-4xl mx-auto">
        ...
        <script>
            function programacionForm(config) {
                return {
                    ...
                    loadCourses() { /* ...código sin cambios... */
                        ...
                        console.log('Alpine: loadCourses - Fetching:', url);
                        fetch(url)
                    ...
                    handleCourseChange() { /* ...código sin cambios... */
                    ...
                    loadInstructors() { /* ...código sin cambios... */
                        ...
                        console.log('Alpine: loadInstructors - Fetching:', url);
                        fetch(url)
                    ...
                    calculateEndDateAndCheckAvailability() { /* ...código sin cambios... */
                        console.log('Alpine: calculateEndDateAndCheckAvailability');
                        this.calculateEndDate();
                    },
                    calculateEndDate() { /* ...código sin cambios... */
                        this.endDate = ''; this.endTime = '';
                        ...
                        console.log("Alpine: calculateEndDate - Llamando API:", this.rutasApi.calcularFechaFin);
                    ...
                    checkAvailability(type) { /* ...código sin cambios... */
                        ...
                        console.log(`Alpine: CheckAvailability (${type}) - Fetching:`, url);
                        fetch(url)
                    ...
                    // --- Modal de Disponibilidad (CON CORRECCIÓN EN $nextTick) ---
                    openAvailabilityModal(type) {
                        ...
                        if (!modal || typeof modal.showModal !== 'function') { console.error("Alpine: Modal <dialog> no encontrado."); return; }
                        ...
                        fetch(url)
                            ...
                            .then(data => {
                                ...
                                requestAnimationFrame(() => {
                                    console.log("Alpine: openAvailabilityModal (rAF) - Renderizando...");
                                    try {
                                        ...
                                        if (!currentCalendarEl) throw new Error('#mini-calendar no encontrado.');
                                        ...
                                        console.log("Alpine: openAvailabilityModal (rAF) - Intentando renderizar tabla...");
                                        ...
                                        // --- CORRECCIÓN: Mover isLoadingAvailability = false aquí ---
                                        this.isLoadingAvailability = false;
                                        console.log("Alpine: openAvailabilityModal (rAF): isLoadingAvailability puesto a false.");

                                        // --- CORRECCIÓN: Asegurar cierre de $nextTick ---
                                        this.$nextTick(() => {
                                            console.log("Alpine: openAvailabilityModal (rAF): $nextTick ejecutado.");
                                        }); // <-- Llave de cierre para $nextTick
                                        ...
                                        // --- Asegurarse de ocultar "Cargando" también en caso de error ---
                                        this.isLoadingAvailability = false;
                                        console.log("Alpine: openAvailabilityModal (rAF) - ERROR: isLoadingAvailability puesto a false.");
                                    }
                            ...
                            .catch(error => {
                                ...
                                if (calendarEl) calendarEl.innerHTML = `<p class="text-red-500 p-4">Error API</p>`;
                                if (tablaDetalleContenido) tablaDetalleContenido.innerHTML = `<p class="text-red-500 p-4">Error al cargar: ${error.message}</p>`;
                            });
                    }, // Fin openAvailabilityModal

                    submitForm(event) { /* ...código sin cambios... */
        ...
        <div class="bg-white p-6 rounded shadow-md"
             x-data="programacionForm({
                 ...
                 // Aquí podrías pasar datos para edición si esta fuera la vista edit.blade.php
                 // isEditing: false, // O true si es edición
                 // programacionId: null, // O el ID si es edición
                 // initialData: null // O los datos si es edición
             })"
