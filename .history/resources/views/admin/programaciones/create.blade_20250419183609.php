        ...
        <script>
            function programacionForm(config) {
                return {
                    ...
                    isModalOpen: false,         // Para modal disponibilidad
                    isEventModalOpen: false,    // Para modal detalle evento
                    ...
                    // --- Modal de Disponibilidad (Usando DIV y estado Alpine) ---
                    openAvailabilityModal(type) {
                        ...
                        // Referencias a elementos DENTRO del modal (ahora un DIV)
                        const modalDiv = document.getElementById('modalDisponibilidad'); // El DIV contenedor
                        const calendarEl = document.getElementById('mini-calendar');
                        const tablaDetalleContenido = document.getElementById('tabla-detalle-contenido');
                        const modalTituloEl = document.getElementById('modalTitulo'); // El H2 del título

                        if (!modalDiv || !calendarEl || !tablaDetalleContenido || !modalTituloEl) {
                            console.error("Alpine: Elementos del modal no encontrados (#modalDisponibilidad, #mini-calendar, #tabla-detalle-contenido, #modalTitulo).");
                            alert("Error: No se puede abrir el modal correctamente.");
                            return;
                        }
                        ...
                        this.isModalOpen = true; // Mostrar el modal DIV usando x-show
                        ...
                        fetch(url)
                            ...
                            .then(data => {
                                ...
                                // Usar requestAnimationFrame para renderizar después de que el modal sea visible
                                requestAnimationFrame(() => {
                                    try {
                                        if (typeof window.FullCalendar === 'undefined' || !window.FullCalendar) throw new Error('FullCalendar no está en window.');
                                        // Re-obtener la referencia por si acaso
                                        const currentCalendarEl = document.getElementById('mini-calendar');
                                        if (!currentCalendarEl) throw new Error('#mini-calendar no encontrado en rAF.');
                                        ...
                                        console.log("Alpine: openAvailabilityModal (rAF) - Tabla OK.");
                                         ...
                                         const errorCalendarEl = document.getElementById('mini-calendar'); // Re-obtener por si acaso
                                         const errorTablaEl = document.getElementById('tabla-detalle-contenido');
                                         if (errorCalendarEl) errorCalendarEl.innerHTML = `<p class="text-red-500 p-4">Error calendario: ${renderError.message}</p>`;
                                         if (errorTablaEl) errorTablaEl.innerHTML = `<p class="text-red-500 p-4">Error tabla.</p>`;
                                    } finally {
                            ...
                            .catch(error => {
                                 ...
                                 const errorCalendarEl = document.getElementById('mini-calendar');
                                 const errorTablaEl = document.getElementById('tabla-detalle-contenido');
                                 if (errorCalendarEl) errorCalendarEl.innerHTML = `<p class="text-red-500 p-4">Error API</p>`;
                                 if (errorTablaEl) errorTablaEl.innerHTML = `<p class="text-red-500 p-4">Error al cargar: ${error.message}</p>`;
                            });
