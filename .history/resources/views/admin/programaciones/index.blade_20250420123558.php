<x-app-layout>
    <div class="py-6 max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-4">
            <h1 class="text-2xl font-bold flex items-center gap-4 flex-wrap">
                Programaciones
                <form method="GET" action="{{ route('admin.programaciones.index') }}" class="flex gap-2 items-center text-sm">
                    <label for="mes">Mes</label>
                    <select name="mes" id="mes" class="border px-3 py-1.5 rounded w-28" onchange="this.form.submit()">
                    ...
                    <select name="anio" id="anio" class="border px-3 py-1.5 rounded w-24" onchange="this.form.submit()">
            ...
            <form method="GET" action="{{ route('admin.programaciones.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                @if(auth()->user()->esAdministrador() && is_null(auth()->user()->coordinacion_id))
                    <div class="md:col-span-1">
                        ...
                        <select name="coordinacion_id" id="coordinacion_id" class="w-full border px-4 py-2 rounded min-w-[16rem]" onchange="this.form.submit()">
                ...
                <div class="md:col-span-1">
                    ...
                    <select name="grupo_id" id="grupo_id" class="w-full border px-4 py-2 rounded min-w-[16rem]" onchange="this.form.submit()">
                ...
                <div class="md:col-span-2 flex items-end justify-end">
                    <div x-data="{ expand: false }" class="relative w-full max-w-xs">
                        <button type="submit" @click.prevent="expand = !expand" class="bg-[#00AF40] text-white px-4 py-2 rounded-full hover:bg-green-700 flex items-center justify-center">
                            <i class="mdi mdi-magnify"></i>
                        </button>
                        <div x-show="expand" @click.away="expand = false" class="absolute top-12 left-0 w-full z-10">
                            <input type="text" name="buscar" id="buscar" placeholder="Buscar" value="{{ request('buscar') }}" class="w-full border px-4 py-2 rounded shadow" />
                        </div>
                    </div>
                </div>
            </form>
