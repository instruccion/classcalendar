<x-app-layout>
    ...
    <div class="py-6 max-w-6xl mx-auto"
         x-data="ordenarBloque({
             ...
             cursosDisponibles: {{ Js::from($grupo->cursos->map(fn($c) => ['id' => $c->id, 'nombre' => $c->nombre, 'duracion_horas' => $c->duracion_horas])) }}
         })" x-init="init()">
                ...
                <div class="mb-4">
                    ...
                    <select @change="agregarCurso($event.target.value); $event.target.value = ''"
                        ...
                        <template x-for="c in cursosDisponibles" :key="c.id">
                            <option :value="c.id" x-text="c.nombre" :disabled="cursos.some(cc => cc.id === c.id)"></option>
                        </template>
                    </select>
                </div>
