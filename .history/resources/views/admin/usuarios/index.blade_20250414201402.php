                    ...
                    @foreach ($users as $user)
                        <tr class="border-t">
                            ...
                            <td class="px-4 py-2 flex gap-2">
                                ...
                                <!-- Eliminar -->
                                <form action="{{ route('users.destroy', $user) }}" method="POST" onsubmit="return confirm('¿Confirmas la eliminación del usuario?')">
                                    @csrf
                                    @method('DELETE')
            ...
            <div class="bg-white p-6 rounded-lg w-full max-w-xl">
                ...
                <form id="createUserForm" method="POST" action="{{ route('users.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="createName" name="name" class="mt-1 block w-full border rounded px-3 py-2" required>
                    </div>

                    <!-- Email -->
                    <div class="mb-4">
                        <label for="createEmail" class="block text-sm font-medium text-gray-700">Email</label>
                        <input type="email" id="createEmail" name="email" class="mt-1 block w-full border rounded px-3 py-2" required>
                    </div>

                    <!-- Rol -->
                    <div class="mb-4">
                        <label for="createRol" class="block text-sm font-medium text-gray-700">Rol</label>
                        <select id="createRol" name="rol" class="mt-1 block w-full border rounded px-3 py-2" required>
                    ...
                    <div class="mb-4 flex items-center">
                        ...
                        <input type="checkbox" id="createIsActive" name="is_active" value="1" class="mr-2">
                        <label for="createIsActive" class="text-sm font-medium text-gray-700">Activo</label>
                    </div>

                    <!-- Contraseña -->
                    <div class="mb-4">
                        <label for="createPassword" class="block text-sm font-medium text-gray-700">Contraseña</label>
                        <input type="password" id="createPassword" name="password" class="mt-1 block w-full border rounded px-3 py-2" required>
                    </div>

                    <!-- Confirmación -->
                    <div class="mb-4">
                        <label for="createPasswordConfirmation" class="block text-sm font-medium text-gray-700">Confirmar Contraseña</label>
                        <input type="password" id="createPasswordConfirmation" name="password_confirmation" class="mt-1 block w-full border rounded px-3 py-2" required>
                    </div>
                    ...
                    <div class="flex justify-end">
                        ...
                        <button type="button" onclick="closeCreateModal()" class="bg-gray-300 text-black px-4 py-2 rounded ml-2">
                            Cancelar
                        </button>
                    </div>
                </form>
