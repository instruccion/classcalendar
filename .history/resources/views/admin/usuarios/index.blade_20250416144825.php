<x-app-layout>
        ...
        <div id="createUserModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm hidden">
            <div class="bg-white w-full max-w-lg rounded-lg shadow-xl p-6">
                ...
                <form id="createUserForm" method="POST" action="{{ route('admin.users.store') }}" class="pt-4 space-y-4">
                    ...
                    <!-- Nombre -->
                    <div>
                        <label for="createName" class="block text-sm font-medium {{ $errors->has('name') ? 'text-red-600' : 'text-gray-700' }} mb-1">Nombre</label>
                        <input type="text" id="createName" name="name" value="{{ old('name') }}"
                            class="w-full px-3 py-2 border {{ $errors->has('name') ? 'border-red-500' : 'border-gray-300' }} rounded shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                        @error('name')
                    ...
                    <!-- Email -->
                    <div>
                        <label for="createEmail" class="block text-sm font-medium {{ $errors->has('email') ? 'text-red-600' : 'text-gray-700' }} mb-1">Email</label>
                        <input type="email" id="createEmail" name="email" value="{{ old('email') }}"
                            class="w-full px-3 py-2 border {{ $errors->has('email') ? 'border-red-500' : 'border-gray-300' }} rounded shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                        @error('email')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <!-- Rol -->
                    <div>
                        <label for="rol" class="block text-sm font-medium {{ $errors->has('rol') ? 'text-red-600' : 'text-gray-700' }} mb-1">Rol</label>
                        <select id="rol" name="rol" class="w-full px-3 py-2 border {{ $errors->has('rol') ? 'border-red-500' : 'border-gray-300' }} rounded shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    <div>
                        <label for="coordinacion_id" class="block text-sm font-medium {{ $errors->has('coordinacion_id') ? 'text-red-600' : 'text-gray-700' }} mb-1">Coordinación</label>
                        <select id="coordinacion_id" name="coordinacion_id" class="w-full px-3 py-2 border {{ $errors->has('coordinacion_id') ? 'border-red-500' : 'border-gray-300' }} rounded shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div>
                            <label for="password" class="block text-sm font-medium {{ $errors->has('password') ? 'text-red-600' : 'text-gray-700' }} mb-1">Contraseña</label>
                            <input type="password" id="password" name="password" class="w-full px-3 py-2 border {{ $errors->has('password') ? 'border-red-500' : 'border-gray-300' }} rounded shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                            @error('password')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label for="password_confirmation" class="block text-sm font-medium {{ $errors->has('password_confirmation') ? 'text-red-600' : 'text-gray-700' }} mb-1">Confirmar Contraseña</label>
                            <input type="password" id="password_confirmation" name="password_confirmation" class="w-full px-3 py-2 border {{ $errors->has('password_confirmation') ? 'border-red-500' : 'border-gray-300' }} rounded shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            @if ($errors->any())
                // Si hay errores de validación, abrir el modal
                openCreateModal();
            @endif
        });
    </script>
