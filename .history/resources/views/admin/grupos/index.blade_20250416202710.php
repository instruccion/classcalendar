            ...
            <form action="{{ route('admin.grupos.store') }}" method="POST" class="grid grid-cols-1 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="nombre" value="{{ old('nombre') }}"
                        class="w-full border px-4 py-2 rounded @error('nombre') border-red-500 @enderror">
                    @error('nombre')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>

                {{-- Solo mostrar la coordinación si es administrador --}}
                @if(auth()->user()->rol === 'administrador')
                    <div>
                        <label class="block font-semibold mb-1">Coordinación</label>
                        <select name="coordinacion_id"
                            class="w-full border px-4 py-2 rounded @error('coordinacion_id') border-red-500 @enderror">
