...
<body class="font-sans antialiased">
    <div class="min-h-screen bg-gray-100 dark:bg-gray-900 flex"> {{-- Cambiado a flex --}}
        ...
        {{-- ======================= --}}
        <aside id="sidebar"
               class="fixed lg:relative top-0 left-0 bottom-0 flex flex-col bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 pt-16 lg:pt-4 p-4 z-40 {{-- Añadido z-40 y padding top para móvil --}}
                      w-64 {{-- Ancho por defecto --}}
                      transform lg:translate-x-0 {{-- Control inicial móvil/desktop --}}
                      transition-all duration-300 ease-in-out {{-- Transición --}}
                      overflow-y-auto {{-- Scroll si es necesario --}}
                      ">
            {{-- Contenedor del Logo (Opcional) --}}
            <div class="sidebar-logo-container flex items-center justify-center h-16 lg:h-auto mb-4 flex-shrink-0">
            ...
            <h2 class="sidebar-title text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3 px-2 mt-2">Menú</h2>

            {{-- Navegación Principal --}}
            <nav class="flex-grow flex flex-col space-y-1"> {{-- flex-grow para empujar logout abajo --}}
                {{-- Enlaces (Adaptar con tus rutas e iconos MDI) --}}
                ...
                <x-sidebar-link :href="route('calendario.index')" :active="request()->routeIs('calendario.index')" icon="mdi-calendar-month-outline">
                 ...
                 <x-sidebar-link :href="route('admin.programaciones.create')" :active="request()->routeIs('admin.programaciones.create') || request()->routeIs('admin.programaciones.edit')" icon="mdi-calendar-plus">
                    Programar Curso
                </x-sidebar-link>
                 <x-sidebar-link :href="route('admin.programaciones.index')" :active="request()->routeIs('admin.programaciones.index')" icon="mdi-format-list-bulleted-type">
                    Programaciones
                 </x-sidebar-link>
                 {{-- Añadir más enlaces aquí siguiendo el patrón --}}
                ...
                {{-- Sección Admin --}}
                 @if(Auth::user()->esAdministrador())
                    ...
                    <h2 class="sidebar-title text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 px-2">Admin</h2>
                     <x-sidebar-link :href="route('admin.coordinaciones.index')" :active="request()->routeIs('admin.coordinaciones.*')" icon="mdi-map-marker-outline"> Coordinaciones </x-sidebar-link>
                     <x-sidebar-link :href="route('admin.users.index')" :active="request()->routeIs('admin.users.*')" icon="mdi-account-cog-outline"> Usuarios </x-sidebar-link>
                     <x-sidebar-link :href="route('admin.feriados.index')" :active="request()->routeIs('admin.feriados.*')" icon="mdi-calendar-remove-outline"> Días Feriados </x-sidebar-link>
                     <x-sidebar-link :href="route('admin.auditorias.index')" :active="request()->routeIs('admin.auditorias.*')" icon="mdi-clipboard-list-outline"> Auditorías </x-sidebar-link>
                 @endif
                 ...
                 <form method="POST" action="{{ route('logout') }}">
                     ...
                     <a href="{{ route('logout') }}"
                           ...
                           class="sidebar-link flex items-center px-3 py-2.5 text-sm font-medium rounded-md text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white">
                        <i class="mdi mdi-logout text-xl mr-3 flex-shrink-0"></i>
                        <span class="sidebar-link-text">{{ __('Log Out') }}</span>
                     </a>
                 </form>
            </nav>
        </aside>
