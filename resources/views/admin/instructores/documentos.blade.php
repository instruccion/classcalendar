...
<x-app-layout>
                    ...
                    @forelse($instructor->documentos as $doc)
                        <tr class="border-t">
                            ...
                            <td class="px-4 py-2">
                                ...
                                {{-- Crear un array con los datos para el modal --}}
                                @php
                                    $modalData = [
                                        'pivot_id'          => $doc->pivot->id,
                                        'nombre'            => $doc->nombre,
                                        'fecha_vencimiento' => $doc->pivot->fecha_vencimiento ? \Carbon\Carbon::parse($doc->pivot->fecha_vencimiento)->format('Y-m-d') : null, // Usar null si no hay fecha
                                    ];
                                @endphp
                                {{-- Pasar el array como un objeto JSON a la función JS --}}
                                <button onclick='abrirModalEditar(@json($modalData))'
    ...
    <script>
        function abrirModalEditar(data) {
            ...
            const fechaYmd = data.fecha_vencimiento;

            console.log("Pivot ID extraído:", pivot_id);
            console.log("Tipo de Pivot ID:", typeof pivot_id); // <--- ¡NUEVO LOG!
            ...
            if (!pivot_id || typeof pivot_id !== 'number') {
                console.error("¡ERROR: pivot_id es inválido o no es un número!", pivot_id, "(Tipo:", typeof pivot_id + ")"); // Log más detallado
